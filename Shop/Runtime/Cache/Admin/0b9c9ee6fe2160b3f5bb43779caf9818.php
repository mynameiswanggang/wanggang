<?php if (!defined('THINK_PATH')) exit();?>
 ...
 <!--循环出菜单开始 -->
    	<?php if(is_array($userMenu)): foreach($userMenu as $key=>$menu): if(($menu["subs"]) == "0"): ?><li class="<?php echo ($menu["class"]); ?>"><a  class="<?php if($key==$current) echo ' active '; ?>" href="#" title=""><span><?php echo ($menu["name"]); ?></span></a></li>
    		 <?php else: ?>
    		 <li class="<?php echo ($menu["class"]); ?>"><a href="#" class="<?php if($key==$current) echo ' active '; ?>exp" title="" ><span><?php echo ($menu["name"]); ?></span><strong><?php echo ($menu["subs"]); ?></strong></a>
    		<ul class="sub">
    		<?php if(is_array($menu)): foreach($menu as $key=>$sub): if((is_array($sub)) == "true"): ?><li><a href="/index.php/Admin/<?php echo ($sub["url"]); ?>" title=""><?php echo ($sub["name"]); ?></a></li><?php endif; endforeach; endif; ?>
